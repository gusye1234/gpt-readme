...
Dependency: 你需要在这个章节专业的识别总结这个模块包含的'子模块'和'代码文件'之间存在的依赖关系, 并且推理总结'子模块'和'代码文件'的互相依赖组成了什么功能. 同时列举出不属于这个模块的第三方依赖有哪些
Command-line Usage: 请你一步步观察发现'子模块'和'代码文件'的描述中是否含有命令行参数指令来指使如何使用这个代码库. 如果有的话, 请你在这个章节的总结中列举出正确的命令行使用方法, 以及正确的命令行参数选项有哪些. 否则不需要生成这一章节
...
Dependency: 你需要在这个章节总结'代码文件'里包含的外部依赖的类, 变量, 函数有哪些, 并且分别推理总结这些依赖的引用名称或者相对路径, 以及依赖对应的功能是.
Command-line Usage: 如果你发现这个'代码文件'是含有命令行参数指令. 请你在这个文件的总结中列举出正确的命令行使用方法, 以及正确的命令行参数选项有哪些. 如果只是包含函数, 类等代码逻辑的话, 这不需要生成这一章节
...
你需要理解, 总结这个代码库的功能, 并且生成一个容易理解, 专业的README文档. 
...
Acknowledgement: 你需要在这个章节总结这个代码库用到的第三方代码库, 并且分别介绍他们的功能. 在这个章节的最后表示你的感谢
