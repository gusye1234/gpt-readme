...
def dir_summary(path):
    ...
    if envs["cache"] is not None:
        dir_cache = envs["cache"].get(path, None)
    ...
    if envs["cache"] is not None:
        envs["cache"][path] = {}
        envs["cache"][path]["summary"] = dir_result['summary']
        envs["cache"][path]["language"] = dir_result['language']
        envs["cache"][path]["hash"] = hash_dir(path)
